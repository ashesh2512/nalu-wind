...
MomentumEdgeSolverAlgCube::MomentumEdgeSolverAlgCube(
  Realm& realm,
  stk::mesh::Part* part,
...
void
MomentumEdgeSolverAlgCube::execute()
{
  ...
  NALU_ALIGNED DblType vSphere[NDimMax_];
  vSphere[0] = stk::math::sin((2*M_PI) * (realm_.get_current_time()));
  vSphere[1] = 0;
  vSphere[2] = 0;
...
//      const DblType mdot = massFlowRate.get(edge, 0);
      /////////////////////////////////////////////////////////////////
      DblType vDotArea = 0.0;
      for (int d=0; d < ndim; ++d) {
        vDotArea += vSphere[d] * av[d];
      }
      ...
      const DblType mdot = rhoIp * vDotArea;
      /////////////////////////////////////////////////////////////////
